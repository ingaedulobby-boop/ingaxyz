...
export default function AIChatButton({ isOpen, onClick, hasUnread }: AIChatButtonProps) {
  return (
    <motion.button
      ...
      className="fixed bottom-[calc(0.75rem+env(safe-area-inset-bottom))] right-3 z-[51] h-10 w-10 sm:h-11 sm:w-9 rounded-full snprimary text-primary-foreground justify-center- flex items-center justify-center"
      style={{
      ...
      <AnimatePresence mode="wait" initial={false}>
        ...
        <motion.span key="close" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }} transition={{ duration: 0.15 }}>
            <MessageCircle className="w-4 h-4 sm:w-[11px] sm:h-[11px] text-destructive-foreground px-0 mx-0 bg-primary-foreground" />
          </motion.span> :
